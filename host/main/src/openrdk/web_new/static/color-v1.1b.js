...
const COLOR_STUDIO_PAGE_VERSION = document.body.dataset.pageVersion || "1.1";
...
const elements = {
  ...
  configPresetInput: document.getElementById("configPresetInput"),
  ...
  telemetryDeliveryInput: document.getElementById("telemetryDeliveryInput"),
  displayHysteresisInput: document.getElementById("displayHysteresisInput"),
...
function renderSnapshot() {
  ...
  const confidenceMilli = Number(data.confidence_milli || 0);
  const hysteresisMilli = Number(cfg.display_hysteresis_milli ?? 30);
  if (state.displayedSlot !== detectedSlot) {
    state.displayedSlot = detectedSlot;
    state.displayedConfidenceMilli = confidenceMilli;
  } else if (state.displayedConfidenceMilli === null ||
             Math.abs(confidenceMilli - state.displayedConfidenceMilli) >= hysteresisMilli) {
    state.displayedConfidenceMilli = confidenceMilli;
  }
  ...
  elements.currentColorConfidence.textContent = formatPercentFromMilli(state.displayedConfidenceMilli);
...
function renderConfigInputs() {
  ...
  elements.telemetryDeliveryInput.value = String(cfg.telemetry_delivery_mode ?? 0);
  elements.displayHysteresisInput.value = (Number(cfg.display_hysteresis_milli ?? 30) / 10).toFixed(1);
  elements.calibrationModeSelect.value = String(cfg.palette_mode);
}

function applyConfigPreset(preset) {
  if (preset === "fast") {
    elements.samplePeriodInput.value = "20";
    elements.gainModeInput.value = "0";
    elements.integrationInput.value = "50";
    elements.confidenceInput.value = "0.10";
    elements.patchSamplesInput.value = "12";
  } else if (preset === "medium") {
    elements.samplePeriodInput.value = "60";
    elements.integrationInput.value = "150";
    elements.patchSamplesInput.value = "12";
    elements.telemetryDeliveryInput.value = "0";
  } else if (preset === "long") {
    elements.samplePeriodInput.value = "80";
    elements.integrationInput.value = "240";
    elements.confidenceInput.value = "0.30";
    elements.patchSamplesInput.value = "20";
    elements.telemetryDeliveryInput.value = "0";
  }
}
...
async function updateConfigFromInputs() {
  ...
  const payload = {
    ...
    telemetry_delivery_mode: Number(elements.telemetryDeliveryInput.value),
    display_hysteresis_milli: Math.round(Number(elements.displayHysteresisInput.value) * 10),
  };
...
async function persistConfig() {
  ...
  elements.configSaveStatus.textContent = "Saved configuration.";
}
...
async function toggleLed() {
  if (!state.selectedSerial || !state.snapshot?.cfg) return;
  const nextMode = Number(state.snapshot.cfg.led_mode) === 0 ? 1 : 0;
  const payload = await apiPost(
    `/api/devices/${encodeURIComponent(state.selectedSerial)}/color/config`,
    { led_mode: nextMode },
  );
  ...
  renderSnapshot();
  renderConfigInputs();
  appendFeed(elements.eventFeed, state.eventLines, `LED ${nextMode ? "on" : "off"}.`);
}
...
elements.deviceSelect.addEventListener("change", async (event) => {
  ...
  state.eventLines = [];
  state.telemetryStartedHere = false;
  state.displayedConfidenceMilli = null;
  state.displayedSlot = null;
...
elements.configPresetInput.addEventListener("change", () => {
  applyConfigPreset(elements.configPresetInput.value);
  elements.configSaveStatus.textContent = "Preset loaded. Review the fields, then apply and save.";
});
[
  elements.sensorNameInput,
  elements.paletteModeInput,
  elements.samplePeriodInput,
  elements.ledModeInput,
  elements.gainModeInput,
  elements.gainInput,
  elements.integrationInput,
  elements.classifierInput,
  elements.confidenceInput,
  elements.targetClearInput,
  elements.patchSamplesInput,
  elements.telemetryDeliveryInput,
  elements.displayHysteresisInput,
].forEach((input) => input.addEventListener("input", () => {
  elements.configPresetInput.value = "custom";
}));
...
elements.persistConfigBtn.addEventListener("click", () => persistConfig().catch((err) => {
  elements.configSaveStatus.textContent = `Save CFG failed: ${err.message}`;
...
elements.toggleLedBtn.addEventListener("click", () => toggleLed().catch((err) => {
  appendFeed(elements.eventFeed, state.eventLines, `LED toggle failed: ${err.message}`);
}));
...
elements.persistCalibrationBtn.addEventListener("click", () => apiPost(
  ...
  setCalibrationStatus("Saved calibration.");
  appendFeed(elements.eventFeed, state.eventLines, "Calibration saved to firmware NVS.");
}).catch((err) => {
  setCalibrationStatus(`Save CAL failed: ${err.message}`);
