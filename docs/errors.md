...
**Raised by:** `require_device()`, module constructors, `send_raw_cmd()`, `send_raw_control()`, `send_raw_traction()`, `refresh()`, `flash_firmware()`.
...
**Raised by:** `send_raw_cmd()`, `send_raw_control()`, `send_raw_traction()`, and any method that calls `_ensure_online()` internally (all movement and query methods on `TractionModule` and `LineSensorModule`).
...
**Raised by:** any method that sends a command and checks the response — `send_raw_cmd()`, `send_raw_control()`, `send_raw_traction()`, `get_position_telemetry()`, `get_position_pid()`, `get_pid_rpm()`.
