...
const COLOR_STUDIO_PAGE_VERSION = document.body.dataset.pageVersion || "1.2a";
...
function renderSnapshot() {
  ...
  if (!data) {
    elements.currentColorName.textContent = "Watching for telemetry";
    elements.currentColorConfidence.textContent = "Click Start Telemetry to request readings";
    ...
    elements.sensorStateText.textContent = "No reading requested by this page";
...
async function refreshSetup() {
  ...
  const payload = await apiGet(`/api/devices/${encodeURIComponent(state.selectedSerial)}/color/setup`);
  const previousData = state.snapshot?.data || null;
  state.snapshot = { ...payload, data: previousData };
  state.profile = payload.profile || state.profile;
  state.calibration = { cfg: payload.cfg, cal: payload.cal, profile: state.profile };
  renderSnapshot();
  renderConfigInputs();
  renderPaletteEditor();
  renderSingleTargetOptions();
...
async function updateConfigFromInputs() {
  ...
  const payloadResponse = await apiPost(
    `/api/devices/${encodeURIComponent(state.selectedSerial)}/color/config?lean=true`,
    payload,
  );
  state.snapshot = { ...(state.snapshot || {}), cfg: payloadResponse.cfg };
  renderSnapshot();
  renderConfigInputs();
  renderSingleTargetOptions();
...
async function persistConfig() {
  ...
  await apiPost(`/api/devices/${encodeURIComponent(state.selectedSerial)}/color/save?lean=true`, {
    persist_cfg: true,
    persist_cal: false,
  });
...
async function runSelftest() {
  ...
  const payload = await apiPost(`/api/devices/${encodeURIComponent(state.selectedSerial)}/color/selftest?lean=true`, {});
...
async function startCalibrationSession(modeKey) {
  await apiPost(`/api/devices/${encodeURIComponent(state.selectedSerial)}/color/config?lean=true`, {
    palette_mode: Number(modeKey),
  });
  await apiPost(`/api/devices/${encodeURIComponent(state.selectedSerial)}/color/calibration/start?lean=true`, {});
}
...
async function captureTarget(target, label) {
  ...
  await apiPost(`/api/devices/${encodeURIComponent(state.selectedSerial)}/color/calibration/select?lean=true`, {
    target,
  });
  if (state.snapshot?.data) state.snapshot.data.calibration_samples = 0;
  ...
  const payload = await apiPost(`/api/devices/${encodeURIComponent(state.selectedSerial)}/color/calibration/commit?lean=true`, {
    target,
  });
  if (payload.cal && state.snapshot) state.snapshot.cal = payload.cal;
  state.profile = payload.profile || state.profile;
  renderSnapshot();
  renderPaletteEditor();
...
async function captureSelectedTarget() {
  ...
  if (!state.telemetryStartedHere) {
    setCalibrationStatus("Start Telemetry before capturing calibration references or colors.");
    return;
  }
  ...
  try {
    ...
    await apiPost(`/api/devices/${encodeURIComponent(state.selectedSerial)}/color/calibration/stop?lean=true`, {});
    await refreshSetup();
  }
}
...
elements.refreshSnapshotBtn.addEventListener("click", () => {
  refreshSetup().catch((err) => {
    appendFeed(elements.eventFeed, state.eventLines, `Setup refresh failed: ${err.message}`);
  });
});
...
elements.persistCalibrationBtn.addEventListener("click", () => apiPost(
  `/api/devices/${encodeURIComponent(state.selectedSerial)}/color/save?lean=true`,
...
function selectManualExposure() {
  if (elements.gainModeInput.value !== "0") {
    elements.gainModeInput.value = "0";
    appendFeed(
      elements.eventFeed,
      state.eventLines,
      "Manual exposure selected because gain or integration was edited.",
    );
  }
}
elements.gainInput.addEventListener("change", selectManualExposure);
elements.integrationInput.addEventListener("change", selectManualExposure);
