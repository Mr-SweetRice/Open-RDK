  ...
  for (const device of state.devices) {
    ...
    node.innerHTML = `
      <div class="device-title">
        <span class="device-avatar ${kindClass}" title="${escapeHtml(moduleType)}">${escapeHtml(initial)}</span>
        <div class="device-name-block">
          <div class="device-name">${escapeHtml(resolveName(device))}</div>
          <span class="device-module-tag">${escapeHtml(moduleType)}</span>
        </div>
      </div>
