(function(){
  ...
  function scriptOptions(selected=""){
    const references=state.scripts.map(x=>x.reference||x.name);
    let html='<option value="">Selecione um .py</option>';
    if(selected&&!references.includes(selected))html+=`<option value="${escapeHtml(selected)}" selected>Indisponível · ${escapeHtml(selected)}</option>`;
    state.scriptDirectories.forEach(directory=>{
      const scripts=state.scripts.filter(script=>script.directory_id===directory.id);
      if(!scripts.length)return;
      html+=`<optgroup label="${escapeHtml(directory.label)}">`+scripts.map(script=>{const reference=script.reference||script.name;return `<option value="${escapeHtml(reference)}"${reference===selected?" selected":""}>${escapeHtml(script.name)}</option>`}).join("")+"</optgroup>";
    });
    return html;
  }
  ...
  function renderScriptDirectories(){
    el.directoryList.innerHTML=state.scriptDirectories.map(directory=>`<div class="directory-item${directory.available?"":" unavailable"}"><div><strong>${escapeHtml(directory.label)}</strong><small title="${escapeHtml(directory.path)}">${escapeHtml(directory.path)}</small><span>${directory.available?`${directory.script_count} script${directory.script_count===1?"":"s"}`:"Diretório indisponível"}</span></div>${directory.managed?'<span class="directory-badge">Gerenciado</span>':`<button class="action-btn directory-remove" type="button" data-directory-id="${escapeHtml(directory.id)}">Remover</button>`}</div>`).join("");
  }
  async function loadScripts(){const payload=await request("/api/control-hub/scripts");state.scripts=payload.scripts||[];state.scriptDirectories=payload.directories||[];renderScriptDirectories();el.scriptList.innerHTML=state.scripts.length?state.scripts.map(x=>`<span><strong>${escapeHtml(x.name)}</strong><small>${escapeHtml(x.directory_label||"Diretório gerenciado")}</small><em>${x.size>=1024*1024?(x.size/(1024*1024)).toFixed(1)+" MB":Math.max(1,Math.ceil(x.size/1024))+" KiB"}</em></span>`).join(""):"Nenhum código carregado.";[...el.menu.children].forEach(row=>{const select=row.querySelector('[data-role="script"]'),selected=select.value;select.innerHTML=scriptOptions(selected)})}
  ...
  el.addDirectory.addEventListener("click",()=>act(async()=>{const path=el.directoryPath.value.trim();if(!path)throw new Error("Informe o caminho absoluto do diretório.");await request("/api/control-hub/script-directories","POST",{path});el.directoryPath.value="";await loadScripts()},"Diretório de scripts adicionado."));
  el.directoryPath.addEventListener("keydown",event=>{if(event.key==="Enter"){event.preventDefault();el.addDirectory.click()}});
  el.directoryList.addEventListener("click",event=>{const button=event.target.closest("[data-directory-id]");if(!button)return;act(async()=>{await request("/api/control-hub/script-directories/remove","POST",{directory_id:button.dataset.directoryId});await loadScripts()},"Diretório removido. Os demais continuam ativos.")});
